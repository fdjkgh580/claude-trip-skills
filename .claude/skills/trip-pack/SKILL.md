...
1. 讀取**根目錄** `./traveler-profile.md` 取得旅行者畫像（特別注意「國籍」與「護照狀態」欄位）
2. 讀取 `./$TRIP/trip-meta.md` 取得行程概要、出發日期
3. 根據畫像的輸出格式讀取行程表：
...
- 更新記帳檔匯率（確認 `./$TRIP/expense-log.md` 開頭的匯率是否需要調整）
...
補完後寫回**根目錄** `./traveler-profile.md` 對應欄位。血型 / 過敏 / 慢性病若填了，只放緊急卡不進 profile（敏感資料）。

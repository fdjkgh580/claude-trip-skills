...
- 更新記帳檔匯率（確認 `expense-log.md` 開頭的匯率是否需要調整）
...
出發前一週階段，額外產出 `emergency-card.md`（Markdown，A4 可列印時內容足夠排成一張紙）。**不要一開始就產 HTML**：緊急卡資料還可能調整（改住宿、改保險），等使用者確認定稿後說「幫緊急卡出 HTML」再產 `emergency-card.html`。
...
**清單寫入 `checklist.md`**（檔名固定，`/trip` 以此檔存在與否判定「是否已準備清單」）。若行程表為分日拆檔模式，清單仍統一寫入這一個檔（不拆檔）。

若距出發 ≤ 14 天，額外同步產出 `emergency-card.md`（依「紙本緊急卡產出規則」）。HTML 版不自動產，使用者要列印時說一聲才產。

## 完成後

顯示清單摘要（每個階段幾個項目 + 距出發天數），告訴使用者 `checklist.md` 已產出；若有產 `emergency-card.md` 也一併告知，並提醒「要列印就跟我說『出 HTML』」。

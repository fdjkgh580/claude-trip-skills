...
| 距出發天數 < 簽證所需時間（或護照已過期且不夠急件時程） | 🔴 來不及 | **停下來**，用 `AskUserQuestion` 問使用者要怎麼辦：改期 / 改目的地 / 機票已訂需協助辦急件 / 取消；選了再決定要不要繼續研究 |
...
| 機票已訂，請指引我辦急件 | 附急件辦理窗口、所需文件清單、護照急件同步辦法；繼續後續研究，但在報告開頭標紅字警告 |
| 取消這次規劃 | 結束流程 |
| 幫我決定 | 根據距出發天數：< 簽證所需工作天 × 0.5 → 改期、其他 → 急件辦理指引 |
...
完成後接續 R6（當季 F），讀取 R1-R5 結果後再跑，品質更好。
如果使用者已有機票，R3 只做交通。

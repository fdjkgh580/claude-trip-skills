...
若使用者選「幫我確認」但遲遲無法確認（例如回「還是不確定」「找不到」），直接把護照狀態先記為「待確認」往下走，並在回報摘要中提醒「出發前務必再確認護照效期」。不要在這題卡住。
...
### 3.5 緊急聯絡資料（之後要放進紙本緊急卡）

用一般對話收集（不用 AskUserQuestion，都是開放式答案）：

1. **姓名（中文 + 英文護照名）** — 例如「王小明 / WANG, HSIAO-MING」
2. **緊急聯絡人** — 姓名、與你的關係、電話（含國碼，例如 +886 912-345-678）

告訴使用者：「這些資料只會寫進你本機的 `traveler-profile.md`，不會上傳任何伺服器。之後 `/trip-pack` 會用來產紙本緊急卡。」

血型、過敏、慢性病等醫療資訊留到 `/trip-pack` 產紙本緊急卡時再問（那時候才真的會印出來），這裡不收。

...
`/save` 把變更 push 到 GitHub 之後，**GitHub Actions 會在背景自動把資料合併到主線（main）**，整個過程約 5-30 秒。使用者完全不用做任何事。

如果使用者問「我的資料在嗎？」「main 怎麼還沒更新？」之類問題，告訴他「GitHub 那邊正在整理，幾秒後就會好」。**不要解釋 Actions、merge、branch 的技術細節**，使用者不需要知道。

如果超過 1 分鐘 main 還沒更新，可能是 Actions workflow 出問題，告訴使用者「儲存有完成，但雲端整理可能卡住，可以截圖告訴開發者」。

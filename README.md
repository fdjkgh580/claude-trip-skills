...
你不需要每規劃一趟新旅行就複製一個專案。**同一個倉庫底下可以放多個獨立行程**，每個行程是一個自動編號的資料夾：
...
- 資料夾名格式：`{編號}-{地點}-{開始研究的日期}`（編號從 1 自動累加，日期是當天系統日期）
- 地點怎麼寫都可以（城市、國家、區域皆可），不強制指定國家
- 根目錄的 `current-trip` 檔會記住「目前在規劃哪一個」，Claude 每次回覆開頭會顯示「📍 目前在規劃 X」
- 切換行程：跟 Claude 說「切換行程」或「換一個行程」
- 新增行程：跟 Claude 說「新增行程」或直接跑 `/trip-plan`
- 旅行者畫像（`traveler-profile.md`）放在根目錄**所有行程共用**，不用每趟重填
